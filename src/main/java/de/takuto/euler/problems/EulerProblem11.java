...
public class EulerProblem11 implements EulerProblem {
    ...
    private enum Direction {
        RIGHT, DOWN, LEFT_DIAGONAL_DOWN, RIGHT_DIAGONAL_DOWN
    }
    ...
    public String solve() {
        ...
        return "The greatest product of " + MAX_ADJACENT + " numbers is " + product;
    }
    ...
    /**
     * Looks in the given direction from a set of coordinates and returns the product.
     * If this function cant look ahead far enough (> MAX_ADJACENT would be out of bounds)
     * zero is returned.
     *
     * @param x Matrix x coordinate start point
     * @param y Matrix y coordinate start point
    ...
    private long lookDirection(int x, int y, Direction d) {
        ...
        while (counter < MAX_ADJACENT) {
            ...
            if(x >= MATRIX.length) {
                return 0;
            } else if (y >= MATRIX[x].length || y < 0 ) {
                return 0;
            }

            // Is zero? Product will be zero
            if(MATRIX[x][y] == 0) {
                return 0;
            }
